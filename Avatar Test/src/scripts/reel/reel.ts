...
export class Reel {
    ...
    public setResult(data: number[]) {
        ...
        for (let i = 0; i < data.length; i++) {
            let reelSym = this.scene.add.sprite(0, i * this._symHeight, this._availableSymbols[data[i]].reelAnim);
            reelSym.name = this._availableSymbols[data[i]].reelAnim.split("sym")[1];
            this._view.add(reelSym);
        }
